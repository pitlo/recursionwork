...
(* DESCRIPTION: This function takes an int input and outputs that int as a string. It utilizes a pow10 function to help with math and utilizes several sub functions to first convert the int to an int list and then the int list into a char list and finally implode the char list into a string.  EXAMPLE: 13 -> "13" or ~120 -> "~120" *)
...
fun int2list(num,n) =
if num=0 then nil
else if num - pow10(n) > 0 then num div pow10(n) :: int2list(num - (num div pow10(n) * pow10(n)), n-1)
else int2list(num, n-1);

fun list2char(L) =
if null L then nil
else chr(hd L + 48) :: list2char(tl L);

fun int2str(num) = implode(list2char(int2list(num,9)));
